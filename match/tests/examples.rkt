#lang whalesong
; The following tests were uncommented:
;   - mutable list tests
;   - prefab structures tests
;   - c struct tests
;   - tests that provoke errors
; All other tests pass.

(require "../../match.rkt"
         (prefix-in m: "../../match.rkt")
         (for-syntax racket))
...
(define-syntax (comp stx)
  (syntax-case stx ()
    [(mytest tst exp)
     #`(begin
         (displayln (format "test: ~a" '#,(syntax->datum (quote-syntax tst))))
         #,(syntax/loc stx (equal? tst exp)))]))
